  ...
  def solve(self):
    ...
    with tf.Session(config=config) as sess:
      ...
      try:
        ...
        print('test loss:%.4f' % (test_total_accuracy / test_acc_count))
...
class VggNet(object):
  ...
  def forward(self, input):
    out = self.make_layer(input, cfg.net_layers[self.vggname])
...
def input_fn(is_training, common_params, dataset_params):
  ...
  # 这个函数的输入是一个文件的列表和一个record_bytes，之后dataset的每一个元素就是文件中固定
  # 字节数record_bytes的内容。通常用来读取以二进制形式保存的文件，CIFAR10数据集就是这种形式
...
def main_train(_):
  print('this is the vgg11')
  print(cfg.common_params)
  net_name = cfg.common_params['net_name']
  # 获取数据集
